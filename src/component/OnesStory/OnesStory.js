            ...
            {IdStory && story ?
                <Fragment>
                    ...
                    <div style={{wordWrap: "break-word", wordBreak: "break-word"}} className="flex shrink-4">
                        <a className="text-gray-300 dark:text-white" href={story.url} target="_blank"
                           rel="noopener">{`Source: ${story.url}`}</a>
                        {/*<p className="text-gray-700 dark:text-white">{`Date: ${makeDateFormat(story.time)}`}</p>*/}
                    </div>
                </Fragment>
                : <h1 className="font-semibold text-gray-700 dark:text-white">Incorrect ID story in URL. Please check
                    URL.</h1>
            }
